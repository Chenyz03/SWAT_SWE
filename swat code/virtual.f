...
!! sum HRU results for subbasin 
      ...
      !! subbasin averages: water
        ...
        sub_sno(sb) = sub_sno(sb) + sno_hru(j) * hru_fr(j)   !sub_sno (subbasin subbasin snow depth in H2O) is added by Yuzhuang Chen 2021/03/23  hrutot(msub) total # of HRUs in subbasin
...
!! perform subbasin level operations after processing last HRU in subbasin
      if (iihru == hrutot(sb)) then
        ...
        if (subfr_nowtr(sb) > 1.e-6) then
            ...
            sub_latq(sb) = sub_latq(sb) / subfr_nowtr(sb)      !subfr_nowtr: subbasin fraction of watershed area    Yuzhuang Chen
            sub_tileq(sb) = sub_tileq(sb) / subfr_nowtr(sb)   ! sub_fr: fraction of watershed area in subbasin  Yuzhuang Chen
        !    sub_sno(sb) = sub_sno(sb)                       !sub_sno (subbasin subbasin snow depth in H2O) is added by Yuzhuang Chen 2021/03/25
        else
            sub_snom(sb) = 0.0
            sub_sumfc(sb) = 0.0
            sub_sw(sb) = 0.0 
            ...
            sub_latq(sb) = 0.0
            sub_tileq(sb) = 0.0
         !   sub_sno(sb) = 0.0         !sub_sno (subbasin subbasin snow depth in H2O) is added by Yuzhuang Chen 2021/03/25
        end if
...
!        sub_minps(sb) = sub_minps(sb) / sub_fr(sb)
     ...
     &                              ratio = sub_hwyld(ii) / sub_wyld(sb)
        ...
        if (curyr > nyskip) then
         ...
         submono(21,sb) = submono(21,sb) + sub_sno(sb)      ! Yuzhuang Chen 2021/03/23
